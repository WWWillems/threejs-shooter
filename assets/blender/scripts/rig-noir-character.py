...
assert not bpy.data.scenes.get('Noir character workshop'), 'Start authoring in a file without the character workshop'
scene=bpy.data.scenes.new('Noir character workshop')
...
def ellipsoid(name,pos,scale,mat,joint):
 bpy.ops.mesh.primitive_uv_sphere_add(segments=24,ring_count=16,location=xyz(pos));o=bpy.context.object;o.scale=(scale[0],scale[2],scale[1]);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);return finish(o,name,mat,joint)
def box(name,pos,size,mat,joint,bevel=.015):
 ...
 mod=o.modifiers.new('tailored edges','BEVEL');mod.width=bevel;mod.segments=3;bpy.ops.object.modifier_apply(modifier=mod.name);return finish(o,name,mat,joint)
def limb(name,a,b,r1,r2,mat,joint):
 va=Vector(xyz(a));vb=Vector(xyz(b));bpy.ops.mesh.primitive_cone_add(vertices=24,radius1=r1,radius2=r2,depth=(vb-va).length,location=(va+vb)/2);o=bpy.context.object;o.rotation_euler=(vb-va).to_track_quat('Z','Y').to_euler();return finish(o,name,mat,joint)
def shell(name,profile,mat,joint,start=0,end=math.tau,segments=40):
 verts=[];faces=[]
 ...
 o=bpy.data.objects.new(name,mesh);scene.collection.objects.link(o);finish(o,name,mat,joint)
...
shell('fitted double breasted coat',[(.94,.215,.135),(1.02,.225,.145),(1.18,.225,.145),(1.38,.272,.16),(1.48,.27,.13),(1.53,.20,.105)],coat,'Spine')
...
for side in ['L','R']:
 ...
 ellipsoid('polished boot',(x,.09,-.077),(.093,.085,.175),leather,'Foot'+side);box('boot sole',(x,.03,-.08),(.188,.042,.31),leather,'Foot'+side,.024)
...
for sign in [-1,1]:
 o=box('broad notched lapel',(sign*.103,1.405,-.171),(.115,.28,.029),lapel,'Spine');o.rotation_euler[1]=sign*.32
...
ellipsoid('face',(0,1.724,-.025),(.127,.168,.116),skin,'Head')
ellipsoid('jaw',(0,1.652,-.036),(.10,.075,.09),skin,'Head')
ellipsoid('nose',(0,1.717,-.139),(.028,.045,.035),skin,'Head')
for sign in [-1,1]:
 ellipsoid('ear',(sign*.126,1.718,-.006),(.025,.052,.032),skin,'Head')
 box('brow',(sign*.046,1.77,-.126),(.063,.018,.014),hair,'Head',.006)
...
shell('fedora crown',[(1.864,.151,.133),(1.905,.15,.131),(1.972,.126,.117),(2.006,.093,.096)],coat,'Head')
...
bpy.ops.mesh.primitive_uv_sphere_add(segments=40,ring_count=12,location=xyz((0,1.868,-.008)));o=bpy.context.object;o.scale=(.229,.201,.019);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);finish(o,'curved fedora brim',coat,'Head')
...
def pose(kind,t):
 ...
 if crouch:
  ...
  setrot('Spine',-.62);setrot('Head',.28)
...
result={'blend':ROOT+'/assets/blender/noir-character.blend','bones':len(data.bones),'clips':[a.name for a in clips],'vertices':len(body.data.vertices)}
